...
def _plot_f1_scores(
    ...
    bars = ax.barh(
        range(len(names)), f1_scores, color="#1f77b4", alpha=0.85, height=0.6
    )
...
def evaluate_metrics(run_dir: Path, config: SimpleNamespace, output_dir: Path) -> None:
    ...
    checkpoint = torch.load(
        run_dir / "best.ckpt", map_location="cpu", weights_only=True
    )
    state_dict = {
        k.replace("model.", "", 1): v
    ...
    print(
        f"Macro F1: {report['macro avg']['f1-score']:.4f}  |  Weighted F1: {report['weighted avg']['f1-score']:.4f}"
    )
    ...
    _plot_confusion_matrix(
        cm, label_names, output_dir / "confusion_matrix.png", model_label
    )
